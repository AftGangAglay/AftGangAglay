...
static enum aga_result aga_openconf(
		const char* path, struct aga_conf_node* root) {
	...
	if((result = aga_fileattr(fp, AGA_FILE_LENGTH, &attr))) return result;

	aga_conf_debug_file = path;
	return aga_mkconf(fp, attr.length, root);
}

static enum aga_result aga_openout(struct aga_conf_node* root, void** fp) {
...
static enum aga_result aga_build_python(void* out, void* in) {
	...
	if((result = aga_copyfile(out, in, AGA_COPY_ALL))) return result;

	written = fwrite(AGA_PY_END, 1, sizeof(AGA_PY_END) - 1, out);
	if(written < sizeof(AGA_PY_END) - 1) return aga_errno(__FILE__, "fwrite");
 ...
 */
	...
	if(fclose(in) == EOF) return aga_errno(__FILE__, "fclose");

	if(fclose(out) == EOF) return aga_errno(__FILE__, "fclose");
...
static enum aga_result aga_build_iter(
		struct aga_conf_node* input_root, aga_bool_t log,
	...
	for(i = 0; i < input_root->len; ++i) {
		struct aga_conf_node* node = &input_root->children[i];
		...
		for(j = 0; j < node->len; ++j) {
			struct aga_conf_node* child = &node->children[j];

			if(aga_confvar("Kind", child, AGA_STRING, &str)) {
			...
			else if(aga_confvar("Path", child, AGA_STRING, &path)) {
				continue;
			}
			else if(aga_confvar("Recurse", child, AGA_INTEGER, &v)) {
				recurse = !!v;
				continue;
			}
		}
		...
		if((result = fn(path, kind, recurse, pass))) {
			aga_soft(__FILE__, "aga_build_input", result);
			held_result = result;
		}
	}
...
static enum aga_result aga_build_input(
	...
	if((result = aga_fileattr_path(path, AGA_FILE_TYPE, &attr))) return result;

	if(attr.type == AGA_FILE_DIRECTORY) {
		return aga_iterate_dir(
...
enum aga_result aga_build(struct aga_opts* opts) {
	...
	struct aga_conf_node* input_root;
	...
	if((result = aga_conftree_wrap(root.children, &input, 1, &input_root))) {
